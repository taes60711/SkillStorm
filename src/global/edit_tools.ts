...
export class EditTools {
  ...
  /**
   * 將DataBase的圖片格式轉換成實際Url
   * @param url 圖片Url
   * @returns 實際圖片Url
   */
  getRealImgStr(url: string): string {
    if (url.includes(APIHttpController.databaseDomainStr)) {
      return url.replaceAll(
        APIHttpController.databaseDomainStr,
        APIHttpController.domainUrl
      );
    } else {
      return url;
    }
  ...
  getYtvideoID(url: string): string {
    ...
    if (url.includes("/shorts/")) {
      ...
      start =
        url.indexOf('src="https://www.youtube.com/embed/') +
        'src="https://www.youtube.com/embed/'.length;
    } else if (url.includes("https://www.youtube.com/embed/")) {
      start =
        url.indexOf("https://www.youtube.com/embed/") +
