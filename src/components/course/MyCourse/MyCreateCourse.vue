<template>
  <BaseView :apiListFunc="getCourseList" @apiReturnData="handleApiReturnData">
    ...
    <template #rightBody>
      <MyCourseBoard :chageMyCoursePage="chageMyCoursePage"></MyCourseBoard>
    </template>
  </BaseView>
</template>
...
import MyCourseBoard from "@/components/course/MyCourse/MyCourseBoard.vue";
...
const chageMyCoursePage = defineModel("chageMyCoursePage");
...
const getCourseList: (page: number, size: number) => Promise<Course[]> = (
  ...
  return new CourseService().getMyCreateCourse(
    page,
    size,
