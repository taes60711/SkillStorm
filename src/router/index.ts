...
router.beforeEach((to, from, next) => {
  ...
  if (to.meta.requiresAuth && !isAuthenticated) {
    ...
    next(RouterManger.AUTH.WELCOME.path);
  } else if (isAuthenticated && to.path === RouterManger.AUTH.WELCOME.path) {
    // 已登入但訪問首頁，跳轉到 PROFILE 頁面
    console.log("已登入，用戶從首頁跳轉到 PROFILE 頁面");
    next(RouterManger.HOME.PROFILE.INDEX.path);
  } else if (isAuthenticated && to.path === RouterManger.AUTH.LOGIN.path) {
